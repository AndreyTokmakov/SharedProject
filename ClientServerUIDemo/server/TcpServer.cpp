...
namespace server
{
    ...
    ssize_t TcpServer::handleSession(common::Socket clientSocket)
    {
        ...
        std::string payload;
        if (common::Request_Type_Command == request.event_type() &&
            common::Request_CommandType::Request_CommandType_Shutdown == request.cmd_type())
        {
            state = State::Stop;
            payload = "Server is shutting down";
        } else {
            payload = "Response for : {" + request.DebugString() + "}";
        }
